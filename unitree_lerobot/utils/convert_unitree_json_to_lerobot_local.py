...
def create_empty_dataset(
    ...
    *,
    ...
    for cam in cameras:
        shape = (camera_shapes or {}).get(cam, (480, 640, 3))
    ...
    out_dir = Path(root) if root is not None else HF_LEROBOT_HOME / repo_id
    if out_dir.exists():
...
def json_to_lerobot(
    ...
    *,
    ...
    if repo_id is None:
        if push_to_hub:
            raise ValueError("--repo-id is required when --push_to_hub is set")
        repo_id = f"local/{Path(raw_dir).name}"
    ...
    json_dataset = JsonDataset(raw_dir, robot_type)
    sample_cameras = json_dataset.get_item(0)["cameras"]
    camera_shapes = {cam: imgs[0].shape for cam, imgs in sample_cameras.items() if imgs}
